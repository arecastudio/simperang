...
import app.controller.BarangSimpan;
...
public class Barang {
	...
	public VBox vBarang() {
		...
		vbox.getChildren().addAll(judul,new Separator(Orientation.HORIZONTAL),grid,new Separator(Orientation.HORIZONTAL),hbox2,table,new HBox(label));
		return vbox;
	}
				...
				int simpan;
					try {
						if(nama.getStyle().trim().length()>0) {
							simpan=new BarangSimpan(nama.getText().toString().trim(),satuan.getText().toString().trim(),keterangan.getText().toString().trim(),nama.getStyle().toString().trim(),harga.getText().toString().trim()).ProsesUpdate();
						}else {
							simpan=new BarangSimpan(nama.getText().toString().trim(),satuan.getText().toString().trim(),keterangan.getText().toString().trim(),"0",harga.getText().toString().trim()).ProsesSave();
						}
						if (simpan==1) {
			...
			public void handle(ActionEvent event) {
				if(nama.getStyle().trim().length()>0) {
					...
					if (result.get() == ButtonType.OK){
					    if(nama.getStyle().trim().length()>0) {
					    	try {
								int hapus=new BarangSimpan(nama.getText().toString().trim(),satuan.getText().toString().trim(),keterangan.getText().toString().trim(),nama.getStyle().toString().trim(),harga.getText().toString().trim()).ProsesDelete();
								refresh();
							} catch (SQLException e) {
